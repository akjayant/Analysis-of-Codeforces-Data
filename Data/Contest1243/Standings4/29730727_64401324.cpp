...
void solve(){
      ...
      for(j=1;(j*j)<=N;j++)
      {
          if(N%j==0){ vec.pb(j); if((j*j)!=N){vec.push_back(N/j);}}
      }
      ll kash=vec.ss(); ll koulick=0;
      for(j=0;j<kash;j++)
      {
          if(vec[j]!=1){ koulick=__gcd(koulick,vec[j]);}
      }
      koulick=max(koulick,1LL); cout<<koulick<<endl;
}
int main()
{
    fio;
