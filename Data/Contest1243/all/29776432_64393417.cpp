...
int main()
{
    ...
    ip(n);
    vector<ll>Long;
    for(ll iteratorr=2; iteratorr*iteratorr<=n; iteratorr++)
    if(n%iteratorr == 0) 
        {Long.pb(iteratorr);if (iteratorr*iteratorr!=n)Long.pb(n / iteratorr);}
    Long.pb(n); 
    ll z = 0;
    f(iteratorr, Long.size())z = GCD(z, Long[iteratorr]);
    op(z);
}
