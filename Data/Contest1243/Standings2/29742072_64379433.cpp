...
int T, Q, cn;//}
const int inf = 1000000007, mxn = 10005;

char s[mxn], t[mxn], ss[mxn], tt[mxn];
int n;

void must(bool x) { if (!(x)) cout << "No\n", exit(0); }
void done(bool x) { if (x) cout << "Yes\n", exit(0); }
...
signed main() { ios::sync_with_stdio(0); cin.tie(NULL); cout.precision(11);
    ...
    while (cn++!=T) {
        ...
        int dif = 0;
        for (int i = 0; i < n; i++) {
            if (s[i]!=t[i]) dif++, ss[dif] = s[i], tt[dif] = t[i];
        }
        cout << (dif==2 and ss[1]==ss[2] and tt[1]==tt[2] ? "Yes\n" : "No\n");
    }
}