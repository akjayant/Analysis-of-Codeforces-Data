...
class UnionFind{
	vi p,s;
	void extend(int N){foor(i,p.size(),N){p<<i;s<<1;}}
	public:UnionFind(void){}
	UnionFind(int N){extend(N-1);}
	int find(int i){extend(i);return p[i]=p[i]==i?i:find(p[i]);}
...
//			if(s[a]>s[b]){swap(a,b);}
	...
	bool same(int a,int b){extend(a);extend(b);return find(a)==find(b);}
	bool same(pii p){return same(p.first,p.second);}
	int size(int x){extend(x);return s[find(x)];}
};
...
int main(){cin.tie(0);ios::sync_with_stdio(false);
	...
	sort(all(I),[&](int a,int b){return E[a].size()<E[b].size();});
	vector<set<int>>vsi(N);
	fr(i,N)vsi[i]<<-1;
	UnionFind uf;
	for(int i:I){
		int fi=uf.find(i);
		if(vsi[fi].size()&&*vsi[fi].begin()==-1){
			vsi[fi].erase(-1);
			fr(j,N){
				if(!uf.same(i,j)){
					vsi[fi]<<j;
		...
		for(auto it=vsi[fi].begin();it!=vsi[fi].end();){
			if(E[i].find(*it)==E[i].end()){
				uf.unite(*it,fi);
				it=vsi[fi].erase(it);
			}else{
				++it;
