...
int main() {
	...
	vector<pair<ll, P>> e;
	REP(i, n) {
		e.push_back(make_pair(c[i], make_pair(0, i + 1)));
	}
	REP(i, n-1) {
		rep(j, i + 1, n) {
			ll cost = k[i] + k[j];
			cost *= (abs(xy[i].first - xy[j].first) + abs(xy[i].second - xy[j].second));
			e.push_back(make_pair(cost, make_pair(i + 1, j + 1)));
		}
	}
	...
	vector<P> ans;
	REP(i, e.size()) {
		ll a = e[i].second.first, b = e[i].second.second;
		if (uni.root(a) != uni.root(b)) {
			uni.unite(a, b);
			cost += e[i].first;
			if (a == 0 || b == 0) {
				inst.push_back(max(a, b));
			}
			else {
