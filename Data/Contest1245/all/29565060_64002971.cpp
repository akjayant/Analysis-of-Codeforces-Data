...
constexpr ll N = 1000026, INF = 1e18, MOD = 1000000007, K = 5;
...
signed main() {
    ...
    string s;
    cin >> s;
    for(char c:s) if(c=='w' || c=='m') return cout << 0 << '\n', 0;
    for(int i = 2; i <= s.size(); i++) f[i]=(f[i-1]+f[i-2])%MOD;
    for(int i = 0, j; i < s.size(); i=j) {
        for(j = i; j < s.size(); j++) if(s[i]!=s[j]) break;
        if(s[i]=='u' || s[i]=='n') ans = ans*f[j-i]%MOD;
    }
    cout<<ans;
}
