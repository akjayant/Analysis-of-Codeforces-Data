...
#define REP(i, n) for (int i = 0, i##_n = (n); i < i##_n; i++)
#define FOR(i, x, y) for (int i = (x), i##_n = (y); i <= i##_n; i++)
#define ROF(i, x, y) for (int i = (x), i##_n = (y); i >= i##_n; i--)
...
namespace IOTech {
  ...
  template <class T> void _W(const V<T>& x) { REP(i, SZ(x)) { _W(x[i]); if (i + 1 < i_n) _W(' '); } }
  void R() {}
...
void Solution() {
  ...
  REP(i, n) REP(j, n) {
    if (i < j) {
      ...
      cand.pb((ll)dis * (k[i] + k[j]));
    }
  }
  REP(i, n) cand.pb(c[i]);
  uniq(cand);
  V<V<P<int, int>>> E(SZ(cand));
  REP(i, n) REP(j, n) {
    if (i < j) {
      int dis = abs(p[i].first - p[j].first) + abs(p[i].second - p[j].second);
      int pos = lower_bound(ALL(cand), (ll)dis * (k[i] + k[j])) - cand.begin();
      E[pos].pb({ i, j });
    }
  }
  REP(i, n) {
    int pos = lower_bound(ALL(cand), c[i]) - cand.begin();
    E[pos].pb({ n, i });
  }
  V<int> anc(n + 1);
  iota(ALL(anc), 0);
  ...
  for (auto& e : S) {
    int x = e.first, y = e.second;
    if (x == n) power.pb(y + 1);
    else wire.pb({ x + 1, y + 1 });
  }
  W(ans), W(SZ(power)), W(power), W(SZ(wire));
  REP(i, SZ(wire)) W(wire[i]);
}
