...
ll dp[40][2][2][2];
ll dpk(string& left,string& right,int i, bool upgood, bool downgood, bool usgood){
    if(i==40){
        return 1;
    }else{
        if(dp[i][upgood][downgood][usgood]!=-1){
            return dp[i][upgood][downgood][usgood];
        }
        ll ans=0;
        // 0 1 case
        bool u=upgood || right[i]=='1';
        bool d=downgood || left[i]=='0';
        if(usgood){
            //cout<<"moo";
            ans+=dpk(left,right,i+1,u,d,usgood);
        }
        if(u && d){
            //cout<<"boo";
            ans+=dpk(left,right,i+1,upgood,downgood,1);
        }
        if(d){
            //cout<<"foo";
            ans+=dpk(left,right,i+1,u,downgood,usgood);
        }
        dp[i][upgood][downgood][usgood]=ans;
        return ans;
    }
}
int main(){
    ...
    if(l==0 && r==0){
        ...
        string lme=bts(40,1);
        string rme=bts(40,r);
        cout<<dpk(lme,rme,0,0,0,0)*2+(2*r+1)<<endl;
        continue;
    }
    string lme=bts(40,l);
    string rme=bts(40,r);
    //cout<<lme<<" "<<rme<<endl;
    cout<<dpk(lme,rme,0,0,0,0)*2<<endl;
    }
}
