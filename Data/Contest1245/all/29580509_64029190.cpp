...
#endif
}namespace vn16{template<typename T>int min_rotation(vector<T>s){int a=0,N=s.size();for(int b=0;b<N;b++)for(int i=0;i<N;i++){if(a+i==b||s[(a+i)%N]<s[(b+i)%N]){b+=max(0,i-1);break;}if(s[(a+i)%N]>s[(b+i)%N]){a=b;break;}}return a;};template<typename T>class StackGuard{private:T x;public:StackGuard(T x):x(x){}~StackGuard(){x();}};}namespace vn16{};
namespace vn16{
const int maxn=2007;
struct TP{
  int i;
...
int main(){
  ...
  cout<<ans<<endl;
  int n1=0;
  for(int i=0;i<n;i++)if(pr[i]==i)n1++;
  cout<<n1<<'\n';
  for(int i=0;i<n;i++)if(pr[i]==i)cout<<(i+1)<<' ';cout<<'\n';
  cout<<(n-n1)<<'\n';
  for(int i=0;i<n;i++)if(pr[i]!=i)cout<<(i+1)<<' '<<(pr[i]+1)<<'\n';
  return 0;
}
};
