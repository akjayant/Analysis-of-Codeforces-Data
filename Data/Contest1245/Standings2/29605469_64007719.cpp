...
void solve(){
    int i, j;
    for(i = r2 = p2 = s2 = 0; i < n; i++){
        r2 += (str[i] == 'R');
        p2 += (str[i] == 'P');
        s2 += (str[i] == 'S');
    }
    int x = min(r1, s2) + min(p1, r2) + min(s1, p2);
    if(x >= (n + 1) / 2){
        mem(vis, false);
        for(i = 0; i < n && r1 > 0; i++){
            if(!vis[i] && str[i] == 'S'){
                res[i] = 'R', vis[i] = true, r1--;
            }
        }
        for(i = 0; i < n && p1 > 0; i++){
            if(!vis[i] && str[i] == 'R'){
                res[i] = 'P', vis[i] = true, p1--;
            }
        }
        for(i = 0; i < n && s1 > 0; i++){
            if(!vis[i] && str[i] == 'P'){
                res[i] = 'S', vis[i] = true, s1--;
            }
        }
        for(i = 0; i < n && r1 > 0; i++){
            if(!vis[i]){ res[i] = 'R'; r1--; vis[i] = true; }
        }
        for(i = 0; i < n && p1 > 0; i++){
            if(!vis[i]){ res[i] = 'P'; p1--; vis[i] = true; }
        }
        for(i = 0; i < n && s1 > 0; i++){
            if(!vis[i]){ res[i] = 'S'; s1--; vis[i] = true; }
        }
        res[n] = '\0';
        ps("YES\n");
        ps(res); nl;
