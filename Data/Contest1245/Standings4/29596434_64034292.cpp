...
priority_queue< T > q;
LL calc(int i , int j){
    return (k[i] + k[j]) * (abs(a[i].fi - a[j].fi) + abs(a[i].se - a[j].se));
}
int main(){
    RD(n);
    while(!q.empty()) q.pop();
    ...
    while(!q.empty()){
        ...
        if (now.y == -1){
            ...
            int faa = ufset::find(now.x);
            int fab = ufset::find(now.y);
            if (faa != fab && (!haspower[faa] || !haspower[fab])){
                ans += now.cost;
                ee.PB(MP(now.x , now.y));
                bool h = haspower[faa] | haspower[fab];
                ufset::unionset(faa , fab);
                int fa = ufset::find(faa);
                haspower[fa] = h;
            }
        }
    }
