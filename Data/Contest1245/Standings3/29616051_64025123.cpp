...
int main() {
    ...
    vector<TP> cands;
    for (LL i = 0; i < n; i++) {
        cands.push_back(make_tuple(c[i], 0, i, i));
    }
    ...
    for (LL i = 0; i < n; i++) {
        for (LL j = 0; j < n; j++) {
            if (i != j) {
                ...
                cands.push_back(make_tuple(cost, 1, i, j));
            }
        }
    }
    ...
    LL sumV = 0;
    vector<LL> ans_v;
    vector<pair<LL, LL>> ans_edges;

    vector<LL> already(n, 0);
    ...
    for (int i = 0; i < cands.size(); i++) {
        ...
        if (which == 0) {
            if (already[UF.find(my)]) continue;
            already[UF.find(my)] = 1;
            ans_v.push_back(my);
            sumV += cost;
        }
        if (which == 1) {
            if (UF.same(my, you)) continue;
            if (already[UF.find(my)] && already[UF.find(you)]) continue;
            bool good = false;
            if (already[UF.find(my)]) good = true;
            if (already[UF.find(you)]) good = true;
            sumV += cost;
            UF.unite(my, you);
            already[UF.find(my)] = good;
            ans_edges.push_back(make_pair(my, you));
        }
    }
    ...
    printf("%d\n", (int) ans_v.size());
    for (int i = 0; i < ans_v.size(); i++) printf("%lld ", ans_v[i] + 1);

    printf("\n");

    printf("%d\n", (int) ans_edges.size());
    for (int i = 0; i < ans_edges.size(); i++) printf("%lld %lld\n", ans_edges[i].first + 1, ans_edges[i].second + 1);
