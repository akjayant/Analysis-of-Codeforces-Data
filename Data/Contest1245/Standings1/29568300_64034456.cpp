...
Jum 01 Nov 2019 10:57:15  WIB
Current Local Time : 22:57:15
...
LL dp(LL pos, bool akiri, bool akanan,bool bkiri, bool bkanan){
    //a true when a bebas
    //b true when b bebas
    if(pos > n) return 1;
    LL &ret = memo[pos][akiri][akanan][bkiri][bkanan];
    if(ret != -1) return ret;
    ret = 0;
    if(akiri || (!akiri && kiri[pos] == 0)){
    // cout << pos << " " << akiri << " " << akanan << " " << bkiri << " " << bkanan << endl;
        // cout << "Masuk" << endl;
        if(bkiri || (!bkiri && kiri[pos] == 0)) ret += dp(pos+1,akiri,(akanan||(kanan[pos] == 1)),bkiri,(bkanan||(kanan[pos] == 1)));
        if(bkanan || (!bkanan && kanan[pos] == 1)) ret += dp(pos+1,akiri,(akanan||(kanan[pos] == 1)),(bkiri||(kiri[pos] == 0)),bkanan);
    }
    if(akanan || (!akanan && kanan[pos] == 1)){
        if(bkiri || (!bkiri && kiri[pos] == 0)) ret += dp(pos+1,(akiri||(kiri[pos] == 0)),akanan,bkiri,(bkanan||(kanan[pos] == 1)));
    }
    return ret;
} 
...
int main(){
    ...
    while(t--){
        ...
        // for(int i = 0;i <= n;i++) cout << kiri[i];
        //     cout << endl;
        // for(int i = 0;i <= n;i++) cout << kanan[i];
        //     cout << endl;
