...
void solve(){
  f[0]=f[1]=1;
  f[2]=2;
  for(int i=3;i<=N1;++i)f[i]=(f[i-1]+f[i-2])%mod;
  string s;cin>>s;
  int n=s.size();
  int ans=1;
  for(char x:s)if(x=='m' || x=='w'){cout<<"0"<<endl;return;}
  int cnt=0;
  char prev='m';
  for(int i=0;i<n;++i){
    if(prev==s[i]){
      ...
      if(prev=='n'  || prev=='u'){(ans*=fib(cnt))%=mod;cnt=1;}
      else cnt=1;
    }
    prev=s[i];
  }
  if(prev=='n' || prev=='u')(ans*=fib(cnt))%=mod;
  cout<<ans<<endl;
}
int32_t  main(){
  IOS;
