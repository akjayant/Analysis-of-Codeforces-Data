...
pii int_to_rc(int x) {
	int r = x/10;
	int c;
	if (r & 1) c = 9 - (x % 10); else c = (x % 10);
	return {r,c};
}

int rc_to_int(pii x) {
	int ans = x.F*10;
	if (x.F & 1) ans += 9 - x.S; else ans += x.S;
	return ans;
}

int main() {
	vector<vector<int>> h(10,vector<int>(10));
	vector<vector<long double>> dp(10,vector<long double>(10));
	...
	for (int i = 1; i <= 99; i++) {
		...
		for (int j = i-1; j >= i-6; j--) {
			if (j < 0) {self++; continue;}
			pii rc = int_to_rc(j);
			int r = rc.F;
			int c = rc.S;
			
			long double toadd = dp[r][c];
			
			if (h[r][c] != 0)
				toadd = min(toadd,dp[r-h[r][c]][c]);
