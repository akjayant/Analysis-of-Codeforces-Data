...
int n;
ll k, a[Maxn], prvsum[Maxn], sufsum[Maxn];
inline bool check(ll d) {
  vector<pair<ll, ll> > v;
  rep1(i, n) {
    v.push_back({a[i] - d, a[i]});
    v.push_back({a[i], a[i] + d});
  }
  for (auto &x: v) {
    ll l = x.fi, r = x.se;
    int c = lower_bound(a + 1, a + n + 1, r, greater<ll>()) - a - 1;
    ll prv = max(prvsum[c] - c * r, 0LL);
    c = upper_bound(a + 1, a + n + 1, l, greater<ll>()) - a;
    ll suf = max((n - c + 1) * l - sufsum[c], 0LL);
    if (prv + suf <= k) return true;
  }
  return false;
}
signed main(int argc, const char *argv[]) {
  ...
  rep1(i, n) prvsum[i] = prvsum[i - 1] + a[i];
  per1(i, n) sufsum[i] = sufsum[i + 1] + a[i];
  ll l = 0, r = a[1] - a[n], ans = a[1] - a[n];
  while (l <= r) {
    ll mid = l + r >> 1;
