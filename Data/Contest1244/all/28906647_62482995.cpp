...
template <class T, class... S> void dbs(string str, T t, S... s) {ll idx = str.find(','); cerr << str.substr(0, idx) << " : " << t << ","; dbs(str.substr(idx + 1), s...);}template <class T> void prc(T a, T b) {cerr << "["; for (T i = a; i != b; ++i) {if (i != a) cerr << ", "; cerr << *i;} cerr << "]\n";}
...
signed main()
{
    ...
    test(t)
    {
        ...
        fr(i,n)
        {
            if(s[i]=='1')
            {
                ma = max(ma,max(n+1,max(2*(i+1),2*(n-i))));
            }
        }
        cout<<ma;
        cout<<"\n";
