...
vll R;
ll dfs(ll curr, ll prev, ll perm_i, vll &perm, vvll &E, vvll &C) {
    if(len(E[curr]) > 2)
        return -INF;

    R[curr] = perm[perm_i];
    ll res = C[curr][perm[perm_i]];
    for(ll nex : E[curr])
        if(nex != prev)
            res += dfs(nex, curr, (perm_i + 1) % len(perm), perm, E, C);
...
yeee main() {
    ...
    vll perm = {0, 1, 2};
    do {
        ll r = dfs(s, -1, 0, perm, E, C);
        if(r >= 0)
            res = min(res, {r, perm});
    } while(next_permutation(ALL(perm)));

    if(res.first == INF || res.first < 0)
        cout << -1 << endl;
    else {
        cout << res.first << endl;
        dfs(s, -1, 0, res.second, E, C);
        FOR(i, 0, N)
            R[i]++;
        cout << R << endl;
    }
