...
// #pragma GCC optimize("unroBigInt-loops")
...
// typedef tree<long long, nuBigInt_type, less<long long>, rb_tree_tag, tree_order_statistics_node_update> pbds;
...
dt nol;
BigInt ca,na,cb,nb,curgcd;

void exgcd(BigInt a, BigInt b){
    if(b == 0){
        nol = {1,0};
        return;
    }
...
int main(){
    ...
    nol.x *= n/curgcd;
    nol.y *= n/curgcd;
    BigInt smallest = nol.y/aa;
    for(BigInt i = smallest+2;i >= smallest-3;i -= 1){
        BigInt curk = i; //any number is a solution for na*x + nb*y = n
        BigInt curx = nol.x+curk*bb;
        BigInt cury = nol.y-curk*aa;
        if(curx >= 0 && cury >= 0 && p >= curx+cury){
            cout << curx << " " << cury << " " << p-(curx+cury) << endl;
            return 0;
        }
    }
    cout << -1 << endl;
