...
ll dp[MM][4][4];
ll dpcall(int p, int x, int fst,int scd)
{
    ll &ret = dp[x][fst][scd];
    if(~ret)return ret;
    ret = 1e15;
    for(auto y:adj[x])
    {
        ...
        for(int i=1;i<=3;i++)
        {
            if(i!=fst && i!=scd)
            {
                ret = min(ret,c[i][y]+dpcall(x,y,scd,i));
            }
        }
    }
    if(ret==1e15)ret = 0;
...
int clr[MM];
void sol_print(int p, int x, ll res,int fst, int scd)
{
    if(res==0)return;
    for(auto y:adj[x])
    {
        ...
        for(int i=1;i<=3;i++)
        {
            if(i!=fst && i!=scd && dp[y][scd][i]+c[i][y]==res)
            {
                clr[y] = i;
                sol_print(x,y,res-c[i][y],scd,i);
                return;
            }
        }
    }
...
int main() {
    ...
    adj[0].pb(root);
    SET(dp);
    ll ans = dpcall(0,0,0,0);
    cout<<ans<<endl;
    sol_print(0,0,ans,0,0);
