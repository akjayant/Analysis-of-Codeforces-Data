...
ll c[N][4], d[N];
ll mn = INFL;
vector<int> od, ans, tmp;
int col[N];

void run(int fi, int se) {
    tmp.clear();
    tmp.push_back(fi);
    tmp.push_back(se);
    while (sz(tmp) < n) {
        int m = sz(tmp);
        tmp.push_back(6 - tmp[m - 1] - tmp[m - 2]);
    }
    ll val = 0;
    rep(i, 0, n - 1) val += c[od[i]][tmp[i]];
    if (val < mn) {
        mn = val;
        ans.resize(n);
        rep(i, 0, n - 1) ans[od[i] - 1] = tmp[i];
    }
}
