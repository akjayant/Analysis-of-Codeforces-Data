...
void solve()
{
	int n, m;
	scanf("%d %d", &n, &m);
	vector <long long> dpB1(max(n, m) + 1, 0), dpB2(max(n, m) + 1, 0);
	vector <long long> dpW1(max(n, m) + 1, 0), dpW2(max(n, m) + 1, 0);
	dpB1[1] = 1; dpW1[1] = 1;
	for (int i = 2; i <= max(n, m); i++)
	{
		dpB2[i] = dpB1[i - 1];
		dpW2[i] = dpW1[i - 1];
		dpB1[i] = (dpW1[i - 1] + dpW2[i - 1]) % MOD;
		dpW1[i] = (dpB1[i - 1] + dpB2[i - 1]) % MOD;
	}
	long long ans = (dpB1[n] + dpB2[n] + dpW1[n] + dpW2[n]) % MOD;
	ans = (ans + dpB1[m] + dpB2[m] + dpW1[m] + dpW2[m]) % MOD;
	ans = (ans - 2 + MOD) % MOD;
	printf("%lld", ans);
}
